...
public class Main {
    ...
    public static double[] pointTest(int w, int h) {
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                map.put(ImmutablePoint.valueOf(col, row), Integer.toString(col) + ", " + Integer.toString(row));
            }
        }
        long end1 = System.nanoTime();
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                map.put(ImmutablePoint.valueOf(col, row), Integer.toString(col) + ",22 " + Integer.toString(row));
            }
        }
    ...
    public static double[] arrayTest(int w, int h) {
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                string[col][row] = Integer.toString(col) + ", " + Integer.toString(row);
            }
        }
        long end1 = System.nanoTime();
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                string[col][row] = Integer.toString(col) + ",22 " + Integer.toString(row);
            }
        }
        long end2 = System.nanoTime();
        return new double[]{(end1 - start1) / 1000000000.0, (end2 - end1) / 1000000000.0};
